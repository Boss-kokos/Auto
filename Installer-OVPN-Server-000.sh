...
function installQuestions() {
	...
	echo ""
	echo "Мне нужно знать IPv4-адрес сетевого интерфейса, который вы хотите использовать для OpenVPN."
	echo "Если ваш сервер находится за NAT, это должен быть ваш общедоступный IPv4-адрес."
	...
	if [[ $APPROVE_IP =~ n ]]; then
		read -rp "Введите IP: " -e -i "$IP" IP
	fi
	if echo "$IP" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		echo ""
		echo "Кажется, этот сервер находится за NAT. Каков его общедоступный IPv4-адрес или имя хоста?"
		echo "Это нужно для того, чтобы клиенты могли подключаться к серверу."
	...
	if eval "$PING6"; then
		echo "У вашего хоста есть возможность подключения IPv6."
		SUGGESTION="y"
	else
		echo "Похоже, что ваш хост не имеет подключения к IPv6."
		SUGGESTION="n"
	fi
	echo ""
	...
	echo "   1) Стандартный: 1194"
	echo "   2) Пользовательский (ввод)"
	echo "   3) Случайный [49152-65535]"
	until [[ $PORT_CHOICE =~ ^[1-3]$ ]]; do
		read -rp "Выбор порта [1-3]: " -e -i 3 PORT_CHOICE
	done
	case $PORT_CHOICE in
	1)
		PORT="1194"
	...
	echo ""
	echo "Какой протокол вы хотите использовать OpenVPN?"
	echo "UDP быстрее. Если он недоступен, вы не должны использовать TCP."
	echo "   1) UDP"
	echo "   2) TCP"
	...
	echo "   1) Текущие системные (from /etc/resolv.conf)"
	echo "   2) Самостоятельный преобразователь DNS (без привязки)"
	echo "   3) Cloudflare (Anycast: по всему миру)"
	echo "   4) Quad9 (Anycast: по всему миру)"
	echo "   5) Quad9 без цензуры (Anycast: по всему миру)"
	echo "   6) FDN (Франция)"
	echo "   7) DNS.WATCH (Германия)"
	...
	until [[ $DNS =~ ^[0-9]+$ ]] && [ "$DNS" -ge 1 ] && [ "$DNS" -le 13 ]; do
		...
		if [[ $DNS == 2 ]] && [[ -e /etc/unbound/unbound.conf ]]; then
			echo ""
			echo "Unbound уже установлен."
			echo "Вы можете разрешить скрипту настраивать его, чтобы использовать его с ваших клиентов OpenVPN."
			echo "Мы просто добавим второй сервер в /etc/unbound/unbound.conf для подсети OpenVPN."
			echo "В текущую конфигурацию не вносятся изменения."
			echo ""

			until [[ $CONTINUE =~ (y|n) ]]; do
				read -rp "Применить изменения конфигурации к Unbound? [y/n]: " -e CONTINUE
			done
			if [[ $CONTINUE == "n" ]]; then
				unset DNS
				unset CONTINUE
			...
			until [[ $DNS1 =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
				read -rp "Основной DNS: " -e DNS1
			done
			until [[ $DNS2 =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
				read -rp "Дополнительный DNS (необязательно): " -e DNS2
				if [[ $DNS2 == "" ]]; then
					break
				fi
			done
	...
	echo "Обратите внимание: что бы вы ни выбрали, все варианты, представленные в сценарии, безопасны. (В отличии от параментров По Умолчанию)."
	echo ""
	...
	if [[ $CUSTOMIZE_ENC == "n" ]]; then
		...
		echo ""
		echo "Выберите, какой сертификат вы хотите использовать:"
		...
		2)
			echo ""
			echo "Выберите размер, который вы хотите использовать для ключа RSA сертификата:"
			echo "   1) 2048 bits (рекоменд.)"
			echo "   2) 3072 bits"
			echo "   3) 4096 bits"
		...
		case $CERT_TYPE in
		1)
			echo "   1) ECDHE-ECDSA-AES-128-GCM-SHA256 (рекоменд.)"
			...
			;;
		2)
			echo "   1) ECDHE-RSA-AES-128-GCM-SHA256 (рекоменд.)"
		...
		echo ""
		echo "Выберите тип ключа Диффи-Хеллмана, который вы хотите использовать.:"
		...
		2)
			echo ""
			echo "Выберите размер ключа Диффи-Хеллмана, который вы хотите использовать:"
			echo "   1) 2048 bits (рекоменд.)"
			echo "   2) 3072 bits"
			echo "   3) 4096 bits"
			until [[ $DH_KEY_SIZE_CHOICE =~ ^[1-3]$ ]]; do
				read -rp "Размер ключа DH [1-3]: " -e -i 1 DH_KEY_SIZE_CHOICE
			done
	...
	echo ""
	echo "Хорошо, это было все, что мне было нужно. Мы готовы настроить ваш сервер OpenVPN прямо сейчас."
...
verb 3" >>/etc/openvpn/client-template.txt
	...
	newClient
	echo "Если вы хотите добавить больше VPN конфигов, вам просто нужно запустить этот скрипт еще раз!"
}

function newClient() {
	echo ""
	echo "Скажи мне название VPN конфига."
	echo "Имя должно состоять из буквенно-цифровых символов. Также может включать подчеркивание или тире.."

	until [[ $CLIENT =~ ^[a-zA-Z0-9_-]+$ ]]; do
		read -rp "Название VPN конфига: " -e CLIENT
	done

	echo ""
	echo "Хотите защитить файл конфигурации паролем?"
	echo "(например зашифровать закрытый ключ с помощью пароля)"
	echo "   1) Без пароля"
	echo "   2) Использовать пароль"
...
function revokeClient() {
	...
	if [[ $NUMBEROFCLIENTS == '0' ]]; then
		echo ""
		echo "У вас нет существующих VPN конфигов!"
		exit 1
	fi

	echo ""
	echo "Выберите существующий VPN конфиг, который вы хотите удалить."
	tail -n +2 /etc/openvpn/easy-rsa/pki/index.txt | grep "^V" | cut -d '=' -f 2 | nl -s ') '
	until [[ $CLIENTNUMBER -ge 1 && $CLIENTNUMBER -le $NUMBEROFCLIENTS ]]; do
		if [[ $CLIENTNUMBER == '1' ]]; then
			read -rp "Выберите один [1]: " CLIENTNUMBER
		else
			read -rp "Выбери один [1-$NUMBEROFCLIENTS]: " CLIENTNUMBER
		fi
	done
...
function removeUnbound() {
	...
	until [[ $REMOVE_UNBOUND =~ (y|n) ]]; do
		echo ""
		echo "Если вы уже использовали Unbound до установки OpenVPN, я удалил конфигурацию, связанную с OpenVPN."
		read -rp "Вы хотите полностью удалить Unbound? [y/n]: " -e REMOVE_UNBOUND
	done
...
function manageMenu() {
	echo "Добро пожаловать в OpenVPN-установку!"
	echo ""
	echo "Похоже, OpenVPN уже установлен."
	echo ""
	echo "Что ты хочешь делать?"
	echo "   1) Добавить новый VPN конфиг"
	echo "   2) Удалить существующий VPN конфиг"
