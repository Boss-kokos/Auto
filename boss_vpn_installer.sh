#!/bin/bash
# =============================================================================
# Установка и настройка VPN-сервера с веб-интерфейсом
# Поддержка: Ubuntu 20.04, 22.04, 24.04, Linux Mint
# Версия: 2.5.0
# Автор: @bossgram | GitHub: https://github.com/Boss-kokos
# Дата генерации: 2025-04-06
# =============================================================================
...
# Файл автоматически сгенерирован скриптом vpn.sh (Автор: @bossgram)
network:
  version: 2
  renderer: networkd
  ethernets:
...
# Настройка DNS
configure_dns() {
    log_info \"Настраиваю DNS через systemd-resolved (Автор: @bossgram)\"
...
# Автор скрипта: @bossgram | https://github.com/Boss-kokos
configure_iptables() {
...
configure_web_interface() {
    log_info \"Настраиваю веб-интерфейс для управления VPN (Автор: @bossgram)\"
...
configure_shellinabox() {
    log_info \"Настраиваю Shell In A Box (Автор: @bossgram)\"
...
configure_ping_daemon() {
    log_info \"Настраиваю демон пинга и сбора системных показателей (Автор: @bossgram)\"
...
configure_metrics_services() {
    log_info \"Настраиваю сервисы метрик и мониторинга (Автор: @bossgram)\"
...
telegram_bot() {
    echo \"Настройка Telegram Bot Service... (Автор: @bossgram)\"
...
configure_home_metrics_daemon() {
    log_info \"Настраиваю Home Metrics Daemon (Автор: @bossgram)\"
...
finalize_setup() {
    log_info \"Выполняю финальные доработки (Автор: @bossgram)\"
    ...
    chmod +x /var/www/html/api/update_network_load.py 2>/dev/null || true
    ...
    chown www-data:www-data /var/log/vpn-web.log 2>/dev/null || true
    chmod -R 755 /home/files
    chmod -R 755 /home/files/.trash/
    chmod -R 755 /home/files/.trash/.tmb/
    chmod -R 755 /var/www/html/data
    chmod 660 /var/log/vpn-web.log 2>/dev/null || true
...
echo \"\"
echo -e \"${BLUE}echo ""
echo -e "${BLUE}        ____                ____  _             ${NC}"
echo -e "${BLUE}       |  _ \              |  _ \| |            ${NC}"
echo -e "${BLUE}       | |_) | ___  _ __  | |_) | | __ _       ${NC}"
echo -e "${BLUE}       |  _ < / _ \| '_ \ |  _ <| |/ _\` |      ${NC}"
echo -e "${BLUE}       | |_) | (_) | |_) || |_) | | (_| |      ${NC}"
echo -e "${BLUE}       |____/ \___/| .__/ |____/|_|\__,_|      ${NC}"
echo -e "${BLUE}                   | |                         ${NC}"
echo -e "${BLUE}                   |_|     Boss-Kokos          ${NC}"
echo""
echo -e \"${YELLOW}==============================================${NC}\"
...
echo \"Автор скрипта: @bossgram | GitHub: https://github.com/Boss-kokos\"
echo \"После перезагрузки сервера все настройки будут применены.\"
echo \"Веб-интерфейс доступен по адресу: http://$LOCAL_IP/\"
echo \"Логин и пароль от веб-сайта совпадает с root\"
echo \"Удачи, боец!\"
exit 0
