...
#ipv6 block
echo "#!/bin/sh
iptables -t nat -D POSTROUTING -s 10.8.0.0/24 -o tun1 -j MASQUERADE
...
echo "#!/bin/sh
ip route add default via 10.10.0.1 table 120
ip rule add from 10.8.0.0/24 table 120
iptables -t nat -A POSTROUTING -o tun1  -j MASQUERADE

exit 0" >/etc/openvpn/upstream-route.sh
...
cp /root/client000.ovpn /root/MineDoubleVPN.ovpn

echo ""
echo ""
echo "=========================================="
echo ""
echo "Готово! Установка выполнена успешно"
echo "Теперь установи второй скрипт на Сервер 2"
echo ""
echo "=========================================="
echo ""
echo ""
