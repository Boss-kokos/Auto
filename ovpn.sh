...
AUTO="${1:-}"
RAND_PORT="$(shuf -i 20000-60000 -n1)"; PORT_DEF="$RAND_PORT"
IPV4_DEF="$(ip -4 route get 1.1.1.1 2>/dev/null | awk '/src/{print $7; exit}')"; [[ -z "${IPV4_DEF:-}" ]] && IPV4_DEF="$(hostname -I | awk '{print $1}')"
DNS_DEF=1
...
menu_dns(){ echo "DNS:"; echo "  1) System"; echo "  2) Unbound (локальный)"; echo "  3) Cloudflare"; echo "  4) Google"; echo "  5) OpenDNS"; echo "  6) Quad9"; if [[ "$AUTO" == "--defaults" ]]; then DNS_CH="$DNS_DEF"; echo "Ваш выбор [$DNS_DEF]: $DNS_DEF"; else read -e -i "$DNS_DEF" -p "Ваш выбор [$DNS_DEF]: " DNS_CH; fi; DNS_CH=${DNS_CH:-$DNS_DEF}; }
menu_mode(){ echo "Режим клиентов:"; echo "  1) Каждый свой конфиг"; echo "  2) Мульти (duplicate-cn)"; if [[ "$AUTO" == "--defaults" ]]; then MODE="1"; echo "Ваш выбор [1]: 1"; else read -e -i "1" -p "Ваш выбор [1]: " MODE; fi; MODE=${MODE:-1}; }
echo "=== Установка OpenVPN ==="
ask PORT "$PORT_DEF" "Порт"; menu_proto; menu_dns; ask PUBLIC_IP "$IPV4_DEF" "Публичный IPv4 для клиентов"; menu_mode
if [[ "$MODE" == "2" ]]; then ask CLIENT "shared" "Имя общего клиента"; else ask CLIENT "client1" "Имя первого клиента"; fi
export DEBIAN_FRONTEND=noninteractive
apt update && apt install -y openvpn easy-rsa iptables-persistent ca-certificates curl
if command -v ufw >/dev/null && ufw status | grep -q "Status: active"; then ufw allow "$PORT/$PROTO" || true; fi
install -d -m 700 /etc/openvpn/easy-rsa && cp -r /usr/share/easy-rsa/* /etc/openvpn/easy-rsa/ && cd /etc/openvpn/easy-rsa
./easyrsa init-pki
...
install_unbound(){ apt install -y unbound; cat >/etc/unbound/unbound.conf.d/openvpn.conf <<UCONF
server:
  interface: 10.8.0.1
...
IFACE="$(ip route show default 2>/dev/null | awk '/dev/{print $5; exit}')"; [[ -z "${IFACE:-}" ]] && IFACE="$(ip route get 1.1.1.1 | awk '/dev/{for(i=1;i<=NF;i++) if($i=="dev"){print $(i+1); exit}}')"
...
install -d /etc/iptables; iptables-save >/etc/iptables/rules.v4 || true; ip6tables-save >/etc/iptables/rules.v6 || true; systemctl enable netfilter-persistent >/dev/null 2>&1 || true
...
gen_client () {
  ...
  { echo "<ca>"; cat /etc/openvpn/server/ca.crt; echo "</ca>";
    ...
    echo "<tls-crypt>"; sed -ne '/BEGIN OpenVPN Static key/,$p' /etc/openvpn/server/tc.key; echo "</tls-crypt>"; } >>"$OUT"
  echo "Создан $OUT"
}
systemctl enable --now openvpn-server@server.service
gen_client "$CLIENT"
echo "Готово. Файлы клиентов: /root/*.ovpn"
[[ "$MODE" == "2" ]] && echo "Включён мульти-конфиг (duplicate-cn). Лучше отдельные клиенты."
echo "Новый клиент: EASYRSA_BATCH=1 /etc/openvpn/easy-rsa/easyrsa build-client-full NAME nopass && bash -c '$(declare -f gen_client); gen_client NAME'"
EOF
