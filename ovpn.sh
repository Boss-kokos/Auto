...
DNS_DEF=1  # 1=System, 2=Unbound(local), 3=Cloudflare, 4=Google, 5=OpenDNS, 6=Quad9
...
menu_dns(){ echo "DNS:"; echo "  1) System"; echo "  2) Unbound (локальный резолвер на VPS)"; echo "  3) Cloudflare 1.1.1.1"; echo "  4) Google 8.8.8.8"; echo "  5) OpenDNS"; echo "  6) Quad9"; if [[ "$AUTO" == "--defaults" ]]; then DNS_CH="$DNS_DEF"; echo "Ваш выбор [$DNS_DEF]: $DNS_DEF"; else read -e -i "$DNS_DEF" -p "Ваш выбор [$DNS_DEF]: " DNS_CH; fi; DNS_CH=${DNS_CH:-$DNS_DEF}; }
...
# -------- client template (с verify-x509-name) --------
...
# -------- генератор .ovpn --------
install -m 755 /usr/local/sbin/ovpn-mkclient <<'MKC'
#!/usr/bin/env bash
set -euo pipefail
NAME="${1:?usage: ovpn-mkclient <name>}"
OUT="/root/${NAME}.ovpn"
...
# -------- менеджер add/revoke --------
install -m 755 /usr/local/sbin/ovpn-manage <<'MAN'
#!/usr/bin/env bash
set -euo pipefail
cmd="${1:-}"; shift || true
case "$cmd" in
  add)
    NAME="${1:?usage: ovpn-manage add <name>}"
    cd /etc/openvpn/easy-rsa
    EASYRSA_BATCH=1 ./easyrsa build-client-full "$NAME" nopass
    /usr/local/sbin/ovpn-mkclient "$NAME"
    ;;
  revoke)
    NAME="${1:?usage: ovpn-manage revoke <name>}"
    cd /etc/openvpn/easy-rsa
    ...
    echo "Отозван $NAME"
    ;;
  *) echo "usage: ovpn-manage {add|revoke} <name>"; exit 1;;
esac
MAN
